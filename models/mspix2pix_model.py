...
class MSPix2PixModel(BaseModel):
    def name(self):
        return 'MSPix2PixModel'
    ...
    def forward(self):
        ...
        fake_B = self.netG(torch.cat((self.real_A, self.real_A), 0), torch.cat(
            (self.z_random1, self.z_random2), 0))
        self.fake_B_random1, self.fake_B_random2 = torch.split(
            fake_B, self.z_random1.size(0), dim=0)

        self.fake_B_random1_condition = torch.cat(
            (self.real_A, self.fake_B_random1), 1)
        self.fake_B_random2_condition = torch.cat(
            (self.real_A, self.fake_B_random2), 1)
    ...
    def backward_G(self):
        ...
        self.loss_G_GAN = self.backward_G_GAN(self.fake_B_random1_condition, self.netD, self.opt.lambda_GAN) + \
            self.backward_G_GAN(self.fake_B_random2_condition,
                                self.netD, self.opt.lambda_GAN)

        # 2, reconstruction |fake_B-real_B|
        if self.opt.lambda_L1 > 0.0:
            self.loss_G_L1 = self.criterionL1(
                self.fake_B_random1, self.real_B) * self.opt.lambda_L1
        else:
            self.loss_G_L1 = 0.0

        # 3, modes seeking loss
        lz = torch.mean(torch.abs(self.fake_B_random2 - self.fake_B_random1)) / \
            torch.mean(torch.abs(self.z_random2 - self.z_random1))
        eps = 1 * 1e-5
        loss_lz = 1 / (lz + eps)
    ...
    def update_D(self):
        ...
        if self.opt.lambda_GAN > 0.0:
            ...
            self.loss_D, self.losses_D = self.backward_D(
                self.netD, self.real_B_condition, self.fake_B_random1_condition, self.fake_B_random2_condition)
