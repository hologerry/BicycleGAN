...
class DualNetModel(BaseModel):
    ...
    def forward(self):
        ...
        self.fake_B = self.fake_C[:, 0, ...] * 0.299 \
        ...
        if self.opt.conditional_D:   # tedious conditoinal data
            self.fake_data = torch.cat([self.real_A, self.fake_C], 1)
            self.real_data = torch.cat([self.real_A, self.real_C], 1)
        else:
            self.fake_data = self.fake_C
            self.real_data = self.real_C
    ...
    def backward_G(self):
        # 1, G(A) should fool D
        self.loss_G_GAN = self.backward_G_GAN(self.fake_data, self.netD, self.opt.lambda_GAN)
