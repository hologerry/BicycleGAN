...
from data.base_dataset import BaseDataset, transform_quad
    ...
    def __getitem__(self, index):
        ...
        if self.opt.nencode > 1:
            ...
            for char in chars_random:
                ...
                Shapes.append(Image.open(s_path).convert('RGB').crop((w, 0, w, h)))
            # for colors
            random.shuffle(self.alphabets)
            chars_random = self.alphabets[:self.opt.nencode]
            for char in chars_random:
                ...
                Colors.append(Image.open(c_path).convert('RGB').crop((w+w, 0, w, h)))
        ...
        A, C, Shapes, Colors = transform_quad(self.opt, A, C, Shapes, Colors)
