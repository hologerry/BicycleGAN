...
class BgFewFusionDataset(BaseDataset):
    """Few shot dataset
    """
    @staticmethod
    ...
    def initialize(self, opt):
        ...
        if self.opt.phase == 'train':
            self.dir_ABC = os.path.join(opt.dataroot, 'style')
        else:
            self.dir_ABC = os.path.join(opt.dataroot, self.opt.phase)
    ...
    def __getitem__(self, index):
        ...
        for char in chars_random:
            ...
            style_path = phase_path.replace('style', 'style')
    ...
    def __len__(self):
        if self.opt.phase == 'train':
            return len(self.few_alphas)
        else:
            return len(self.ABC_paths)

    def name(self):
        return 'BgFewFusionDataset'
